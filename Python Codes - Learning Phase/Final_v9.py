...
def ShowChoice():
    ...
    print("A variável atual possui a seguinte palavra: {0}".format(aguarde_var.get()))
    # if aguarde_var.get() == 'PROSSIGA':
    ...
    # Armazenament_Dados_Leitura()
    # botao1()
    deselect_function()

def Armazenament_Dados_Leitura():
    # Início da lógica de salvamento de dados
    folder_savefile = FilePath_String.get()
    file_savedata = open(folder_savefile + "/" + "savefile.txt", "r+")

    # Header
    header = "Operação; \t Leitura_1; \t Leitura_2; \t Leitura_3; \t Leitura_4; \t Leitura_5; \t Leitura_6;" \
             " \t Leitura_7; \t Leitura_8; \t Leitura_9; \t Leitura_10; \t Leitura_11; \t Leitura_12;"
    ...
    text_line = "{0}; \t {1}; \t {2}; \t {3}; \t {4}; \t {5}; \t {6}; \t {7}; \t {8}; \t {9}; \t {10}; \t {11}; \t {12}".format(
        str(radio_var_1.get()),
...
def deselect_function():
    return # Estou ignorando o deselect button por agora para pdoer visualizar em qual parte está o seletor
    global radio_button_tkinter
    for i in range(len(radio_button_tkinter)):
        radio_button_tkinter[i].deselect()
...
# https://stackoverflow.com/questions/7393430/how-can-i-dynamic-populate-an-option-widget-in-tkinter-depending-on-a-choice-fro/7403530#7403530
    root.after(5000, update_optionmenu)

update_optionmenu() # https://stackoverflow.com/questions/474528/how-to-repeatedly-execute-a-function-every-x-seconds
...
def update_data():
    global serial_port_status  # Esse valor é uma 'flag' variando de: 0 - Ñ Inicializado, 1 - Inicializado, 2 - Não se pode afirmar nada
    global ser
